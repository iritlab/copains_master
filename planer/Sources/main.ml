...
let () =
  ...
  let returncode = (Sys.command "rm solvedatacp/*.*") in
  
  let usage = "\
Usage: "^Sys.argv.(0)^" [-e ENC] [-insat ENC_FILE] [-s SOLVER] MODEL ACTIONS GOAL [-c CONSTR] [--timeout SECONDS]
...
"
  ...
  and process_arg_alone arg =
    match !model, !actions, !goal with
    | "", _, _  -> model := arg (*  *)
    | _, "", _ -> actions := arg
    | _, _, "" -> goal := arg    
    | _, _, _  -> (Printf.eprintf "Usage: %s [opts] MODEL ACTIONS GOAL (see --help).\n"
                  Sys.argv.(0); exit 1)
  in
  ...
  if !model = "" || !actions = "" || !goal = "" then
    (Printf.eprintf "Usage: %s [opts] MODEL ACTIONS GOAL (see --help).\n" Sys.argv.(0); exit 1);
...
(* Aca hace el parser - VERY IMPORTANT !!!
  if !lint then begin
    (*Printf.printf "ACA 001 \n";*)
...
*)  
  ...
  | _ ->
    if (String.length !inputencoding) > 0 then 
      begin
        (*Printf.printf "ACA 003 \n";*)
        (new Touistplan.t !problem !domain !options 1100 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search
      end
    else
    begin
    (*Printf.printf "ACA 004 \n";*)
  *)    
    match !encoding with
    ...
    | "s" ->  (*Printf.printf "ACA 005 \n";*)
                ...
                (new Planer.t !model !actions !goal 1)#search

    | "m" ->  (*Printf.printf "ACA 005 \n";*)
                (*(new Planer.t !problem !domain !options 100 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search*)
                (new Planer.t !model !actions !goal 2)#search                
    (*| "sat-efa" -> (new Planer.t !problem !domain !options 100 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search
    | "sat-open" -> (new Planer.t !problem !domain !options 103 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search
    | "sat-efa-select" -> (new Planer.t !problem !domain !options 150 !inputencoding solver_code !nodewidth !incrmode !incmin !timeout !verbose)#search
...
and nodewidth = ref 1;
  ...
  let solver_code = match !encoding with
    | "sat" -> (new Planer.t !problem !domain !options 100)#search
    | "model" -> 1
    | v -> failwith "solver unknown (tell the dev)"
  in
        ...
        begin 
          Utils.eprint "[Error %d] Can't exe : rm solvedatacp/*.*\n" returncode; 
        end; 
