...
(* Esto debe ir aparte solo cuando llama al miniSAT para devolver el Modelo
let rec push_lit (lit:Ast.t) (cnf:Ast.t) : Ast.t =
  ...
  | Ta x              -> Ta (push_lit lit x)    
  | Not (Prop x)     -> Or (lit, Not (Prop x))
  | And (x,y)        -> And (push_lit lit x, push_lit lit y)
  | Or (x,y)         ->    Printf.printf "OR OR \n";
                                  Or (lit, Or (x,y))
  | ast ->  failwith ("[shouldnt happen] this doesn't seem to be a formulaW: '" ^ (string_of_ast ~debug:true ast) ^ "'")
*)
...
(*Copains Project - Debe ser empleado por RED process !!! en un file aparte*)
    ...
    | Th x ->
      begin
        ...
        | Top        -> Bottom
        | Bottom     -> Top
        | x          ->   (*Printf.printf "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% XXXX - 222 %s  \n" (Pprint.string_of_ast x) ;*)
                          (*Th (to_cnf x) - JFDX 02/03/2020*)
                          Th x
      end
...
(*---Copains Project*)
    ...
    | Red x ->
      begin
        match x with
        | x          -> to_cnf x |> N.getNNF |> E.getExpand |> S.getSimp |> T.getTra (*|> E.getExpand   *)
      end
    ...
    | Simp x ->
      begin
        match x with
        | x      -> S.getSimp x (*|> S.getSimp2 |> S.getSimp1*)
      end    
  
    | Tra x ->
      (*Printf.printf "aca_Tra \n";*)
      begin
        match x with
        | x      -> T.getTra x         
        (*| x          -> to_cnf x*)
    ...
    | Plusa (x,y) -> let (x,y) = (to_cnf x, to_cnf y)  in 
      begin (* !check_only simplifies by returning a dummy proposition *)
        match x, y with
            | x, y -> Ba (Implies(to_cnf x, to_cnf y))
      end  
