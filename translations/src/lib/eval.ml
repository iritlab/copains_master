...
and eval_ast (env:env) (ast:Ast.t) : Ast.t =
  ...
  | Fx (x,y) -> (match eval_ast x, eval_ast y with		
      | Bool x,Bool y -> Bool (x && y)		
      | x',y' -> raise_type_error2 ast x x' y y' "a 'bool'")		



  | Nnf x -> (match eval_ast x with		
      | Bool x -> Bool (x)		
      | x' -> raise_type_error ast x x' "a 'bool'")      		
  | Simp x -> (match eval_ast x with		
      | Bool x -> Bool (x)		
      | x' -> raise_type_error ast x x' "a 'bool'")            		
  ...
  | Hh x -> (match eval_ast x with		
      | Bool x -> Bool (x)		
      | x' -> raise_type_error ast x x' "a 'bool'")		
  | Ta x -> (match eval_ast x with		
      | Bool x -> Bool (x)		
      | x' -> raise_type_error ast x x' "a 'bool'")     		
  | Ha x -> (match eval_ast x with		
      | Bool x -> Bool (x)		
      | x' -> raise_type_error ast x x' "a 'bool'")
  ...
  | Powerset x -> begin
      Printf.printf "Aca estamos POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET 3333333333333 \n\n\n\n" ;
      ...
      let rec all_combinations_to_set k set = match k with
        ...
        (*| 0 -> AstSet.of_list [Set (AstSet.empty)] - JFD*)
        | 0 -> AstSet.of_list []
  ...
  | e -> raise_with_loc ast ("[shouldnt happen] this expression cannot be expanded AAAX: " ^ string_of_ast e ^"\n")
...
and eval_ast_formula (env:env) (ast:Ast.t) : Ast.t =
  ...
  | Fx (Bottom, _) | Fx (_, Bottom) -> Bottom
  | Fx (Top,x) | Fx (x,Top) -> eval_ast_formula x
  | Fx (x,y) ->  Fx (eval_ast_formula x, eval_ast_formula y)
  | Nnf Top    -> Bottom
  | Nnf Bottom -> Top
  | Nnf x      -> Nnf (eval_ast_formula x)
  | Simp Top    -> Bottom
  | Simp Bottom -> Top
  | Simp x      -> Simp (eval_ast_formula x)
  | Tra Top    -> Bottom
  | Tra Bottom -> Top
  | Tra x      -> Tra (eval_ast_formula x)
  | Tra2 Top    -> Bottom
  | Tra2 Bottom -> Top
  | Tra2 x      -> Tra2 (eval_ast_formula x)  


  | Plus (Bottom, _) | Plus (_, Bottom) -> Bottom
  | Plus (Top,x) | Plus (x,Top) -> eval_ast_formula x
  | Plus     (x,y) -> Plus (eval_ast_formula x, eval_ast_formula y)


  | Plusa (Bottom, _) | Plusa (_, Bottom) -> Bottom
  | Plusa (Top,x) | Plusa (x,Top) -> eval_ast_formula x
  | Plusa     (x,y) -> Plusa (eval_ast_formula x, eval_ast_formula y)
  
  | Plush (Bottom, _) | Plush (_, Bottom) -> Bottom
  | Plush (Top,x) 
  | Plush (x,Top) -> eval_ast_formula x
  | Plush     (x,y) -> Plush (eval_ast_formula x, eval_ast_formula y)  

(* JFDX - 20200415*)
  | Th Top    -> Bottom
  | Th Bottom -> Top
  | Th x      -> Th (eval_ast_formula (Prop (string_of_ast x)))
  | Hh Top    -> Bottom
  | Hh Bottom -> Top
  | Hh x      -> Hh (eval_ast_formula x)
  | Ta Top    -> Bottom
  | Ta Bottom -> Top
  | Ta x      -> Ta (eval_ast_formula x)  
  | Ha Top    -> Bottom
  | Ha Bottom -> Top
  | Ha x      -> Ha (eval_ast_formula x)  

  | Ba Top    -> Bottom
  | Ba Bottom -> Top
  | Ba x      -> Ba (eval_ast_formula x) 

  | Box Top    -> Bottom
  | Box Bottom -> Top
  | Box x      -> Box (eval_ast_formula x) 

  | Bh Top    -> Bottom
  | Bh Bottom -> Top
  | Bh x      -> Bh (eval_ast_formula x)  
    
  | Da Top    -> Bottom 
  | Da Bottom -> Top
  | Da x      -> Da (eval_ast_formula x)  

  | Dia Top    -> Bottom   
  | Dia Bottom -> Top
  | Dia x      -> Dia (eval_ast_formula x)  


  | Dh Top    -> Bottom   
  | Dh Bottom -> Top
  | Dh x      -> Dh (eval_ast_formula x)
  ...
  | Bigand (vars,sets,when_optional,body) ->
    (*Printf.printf "Aca estamos inside the BigAnd .. \n"; *)
      ...
      | [Loc (Var (name,_),loc)],[set] -> (* we don't need the indices because bigand's vars are 'simple' *)
        let rec process_list_set env (set_list:Ast.t list) =
          ...
          | x::xs ->
            ...
            | true  -> (*Printf.printf "Aca estamos dentro del TRUE .. \n";*)
            
                      eval_ast_formula_env env body
            | false ->  (*Printf.printf "Aca estamos dentro del FALSE .. \n"; *)
  ...
  | e -> raise_with_loc ast ("eval + this expression is not a formula: " ^ string_of_ast e ^"\n")
...
and set_to_ast_list (env:env) (ast:Ast.t) :Ast.t list =
  ...
  and has_top_or_bot = function
    ...
    | Nnf x                  -> has_top_or_bot x        
    | Simp x                 -> has_top_or_bot x            
    | Th x                   -> has_top_or_bot x        
    | Hh x                   -> has_top_or_bot x            
