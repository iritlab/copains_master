...
let check_nb_vars_same_as_nb_sets (ast:Ast.t) (vars:Ast.t list) (sets:Ast.t list) : unit =
  let loc = match (List.nth vars 0), List.nth sets ((List.length sets)-1) with
    ...
    | _-> failwith "[shouldn't happen] XXX missing locations in vars/sets"
  in
...
let rec eval ?smt:(smt_mode=false) ?(onlychecktypes=false) ast : Ast.t =
  ...
  and eval_touist_code (env:env) ast :Ast.t =
    ...
    let ic = open_in "type.txt" in
      let line = input_line ic in  (* read line from in_channel and discard \n *)
      print_endline line;          (* write the result to stdout *)
      flush stdout;                (* write on the underlying device now *)
      close_in ic;                  (* close the input channel *) 
    ftype := (int_of_string line);
    dd := ((Parse.parse_sat ~debug_syntax:false ~filename:"" (Pprint.string_of_ast ast)) |> Eval.eval |> Cnf.ast_to_cnf |> Nnf.modDegree);    
    if (!ftype == 1) then begin
                              dd := 2; (* Number of worlds = 3, replace the size of the formula function *)
                              (* Write message to file *)
                              let oc = open_out "num_states.txt" in    (* create or truncate file, return channel *)
                              fprintf oc "%s\n" (string_of_int !dd);   (* write size of the formula *)   
                              close_out oc; 
                          end;

    if (!ftype == 2) then begin
                            let icx = open_in "num_states.txt" in
                            let line = input_line icx in  (* read line from in_channel and discard \n *)
                            print_endline line;          (* write the result to stdout *)
                            flush stdout;                (* write on the underlying device now *)
                            close_in icx;  
                            dd := (int_of_string line);
                          end;
  ...
  | e -> raise_with_loc ast ("this does not seem to be a touist code structure XXX: " ^ string_of_ast ~debug:true e ^"\n")
...
and eval_ast (env:env) (ast:Ast.t) : Ast.t =
  ...
  | Plusa (x,y) -> (match eval_ast x, eval_ast y with
      | Bool x,Bool y -> Bool (x && y)
      | x',y' -> raise_type_error2 ast x x' y y' "a 'bool'")
  | Plush (x,y) -> (match eval_ast x, eval_ast y with
      | Bool x,Bool y -> Bool (x && y)
      | x',y' -> raise_type_error2 ast x x' y y' "a 'bool'")
  | Red x -> (match eval_ast x with
      | Bool x -> Bool (x)
      | x' -> raise_type_error ast x x' "a 'bool'")

  | Tra x -> (match eval_ast x with
      | Bool x -> Bool (x)
      | x' -> raise_type_error ast x x' "a 'bool'")


  | Th x -> (match eval_ast x with
      | Bool x -> Bool (x)
      | x' -> raise_type_error ast x x' "a 'bool'")



  | Ta x -> (match eval_ast x with
      | Bool x -> Bool (x)
      | x' -> raise_type_error ast x x' "a 'bool'")      
  ...
  | Powerset x -> begin
      Printf.printf "Aca estamos POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET 22222222222 \n\n\n\n" ;
...
and eval_ast_formula (env:env) (ast:Ast.t) : Ast.t =
  ...
  (* We consider 'bigand' as the universal quantification; it could be translated as
    ...
    | Bigand (vars,sets,when_optional,body) ->    
     ...
     begin check_nb_vars_same_as_nb_sets ast vars sets;
       ...
       | x::xs,y::ys ->
         eval_ast_formula (Bigand ([ast_without_loc x],[y],None,(Bigand (xs,ys,when_optional,body))))
     end
  ...
  | Prop x ->  let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))] @ [ (Parse.parse_sat ("$j"))]; 
              let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))] @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
              Bigand(!vars2a,!vars2b,None,Parse.parse_sat(x^"($j)"));


  | Not Top    -> Bottom
  | Not Bottom -> Top
  | Not x      -> Not ( Red(eval_ast_formula x))
  
  | Red Top    -> Bottom
  | Red Bottom -> Top
  | Red x      -> Red (eval_ast_formula x)

  | Tra Top    -> Bottom
  | Tra Bottom -> Top
  | Tra x      -> Tra (eval_ast_formula x)  


  | Plusa (Bottom, _) | Plusa (_, Bottom) -> Bottom
  | Plusa (Top,x) 
  | Plusa (x,Top) -> eval_ast_formula x
  | Plusa     (x,y) -> Plusa (Red(eval_ast_formula x), Red(eval_ast_formula y))

  | Plush (Bottom, _) | Plush (_, Bottom) -> Bottom
  | Plush (Top,x) 
  | Plush (x,Top) -> eval_ast_formula x
  | Plush     (x,y) -> Plush (Red(eval_ast_formula x), Red(eval_ast_formula y))  


  | Th Top    -> Bottom
  | Th Bottom -> Top
  | Th x      -> Th ( Tra(eval_ast_formula x))

  | Ta Top    -> Bottom
  | Ta Bottom -> Top
  | Ta x      -> Ta ( Tra(eval_ast_formula x))  
  ...
  | Ba Top    -> Bottom
  | Ba Bottom -> Top
  | Ba x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))] @ [ (Parse.parse_sat ("$j"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))] @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r_($i, $j) => "^string_of_ast(x)^"($j,"^(string_of_int !dd)^")"))

  | Box Top    -> Bottom
  | Box Bottom -> Top
  | Box x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))] @ [ (Parse.parse_sat ("$j"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))] @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r_($i, $j) => "^string_of_ast(x)^"($j)"))
                    

  | Bh Top    -> Bottom
  | Bh Bottom -> Top  
  | Bh x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r_($i) => "^string_of_ast(x)^"($i)"))

  | Da Top    -> Bottom
  | Da Bottom -> Top
  | Da x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigor(!vars2a,!vars2b,None,Parse.parse_sat("r_($i) and "^string_of_ast(x)^"($i)"))

  | Dia Top    -> Bottom
  | Dia Bottom -> Top
  | Dia x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))] @ [ (Parse.parse_sat ("$j"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))] @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r_($i, $j) and "^string_of_ast(x)^"($j)"))  
  ...
  | Dh Top    -> Bottom
  | Dh Bottom -> Top
  | Dh x      ->    let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0.."^(string_of_int !dd)^"])"))];
                    Bigor(!vars2a,!vars2b,None,Parse.parse_sat("r_($i) => "^string_of_ast(x)^"($i)"))
  ...
  | e -> raise_with_loc ast ("evalTra + this expression is not a formula: " ^ string_of_ast e ^"\n")
...
and expand_prop_with_set env name indices_optional =
  ...
  let indices, generated_props = match indices_optional with
    | None   -> (* case (1): proposition without indices (e.g.: it_rains) *)
      [], [UnexpProp (name,None)]
...
and expand_var_name (env:env) (prefix, indices:string * Ast.t list option) =
  ...
  | (x,None)   -> x
  | (x,Some y) ->
    x ^"("^ string_of_ast_list "," (List.map (fun e -> eval_ast env e) y) ^")"
...
and set_to_ast_list (env:env) (ast:Ast.t) :Ast.t list =
  ...
  and has_top_or_bot = function
    ...
    | Red x                  -> has_top_or_bot x    
    | Tra x                  -> has_top_or_bot x      
    | Th x                   -> has_top_or_bot x        
    | Ta x                   -> has_top_or_bot x     
    | Ba x                   -> has_top_or_bot x   
    | Da x                   -> has_top_or_bot x   
    | Bh x                   -> has_top_or_bot x   
    | Dh x                   -> has_top_or_bot x   
      
    | Plusa    (x,y)         -> has_top_or_bot x || has_top_or_bot y    
    | Plush    (x,y)         -> has_top_or_bot x || has_top_or_bot y        
