...
(*Copains Project*)
%token AND OR XOR IMPLIES EQUIV BA BOX BH TH HH TA HA DA DIA DH RED NNF SIMP TRA TRA2 NOT PLUS PLUSA PLUSH
(*---Copains Project*)
...
%inline connectors(T):
  ...
  (*Copains Project*)
  | PLUS x=T COMMA s=T RPAREN {Loc (Plus (x,s),($startpos,$endpos))}  
  | PLUSA x=T COMMA s=T RPAREN {Loc (Plusa (x,s),($startpos,$endpos))}  
  | PLUSH x=T COMMA s=T RPAREN {Loc (Plush (x,s),($startpos,$endpos))}      
  (*
  | x=T  PLUSA     y=T   {Loc (Plusa (x,y),($startpos,$endpos)) }  
  | x=T  PLUSH     y=T   {Loc (Plush (x,y),($startpos,$endpos)) }    
  *)
  | RED           x=T   {Loc (Red x,($startpos,$endpos))}
  | NNF           x=T   {Loc (Nnf x,($startpos,$endpos))}
  | SIMP          x=T   {Loc (Simp x,($startpos,$endpos))}  
  | TRA           x=T   {Loc (Tra x,($startpos,$endpos))}
  | TRA2          x=T   {Loc (Tra2 x,($startpos,$endpos))}  
  | TH            x=T   {Loc (Th x,($startpos,$endpos))}    
  | HH            x=T   {Loc (Hh x,($startpos,$endpos))}     
  | TA            x=T   {Loc (Ta x,($startpos,$endpos))}    
  | HA            x=T   {Loc (Ha x,($startpos,$endpos))}         
  | BA            x=T   {Loc (Ba x,($startpos,$endpos))} 
  | BOX           x=T   {Loc (Box x,($startpos,$endpos))}        
  | BH            x=T   {Loc (Bh x,($startpos,$endpos))} 
  | DA            x=T   {Loc (Da x,($startpos,$endpos))}             
  | DIA           x=T   {Loc (Dia x,($startpos,$endpos))}               
  | DH            x=T   {Loc (Dh x,($startpos,$endpos))}               
  (*---Copains Project*)
