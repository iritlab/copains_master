...
let rec string_of_ast ?(utf8=false) ?(show_var=(fun _ -> "")) ?(debug=false) ?(parenthesis=debug) ast =
  ...
  | Not     x     -> "not " ^ of_ast x
  | Red     x when utf8    -> "" ^ of_ast x
  | Red     x     -> "red " ^ of_ast x
  | Fx (x,y) -> "fx(" ^ of_ast x ^ "," ^ of_ast y ^ ")"    
  | Nnf     x when utf8    -> "§" ^ of_ast x
  | Nnf     x     -> "nnf " ^ of_ast x
  | Simp    x when utf8    -> "£" ^ of_ast x
  | Simp    x     -> "simp " ^ of_ast x
  | Tra     x when utf8    -> "@" ^ of_ast x
  | Tra     x     -> "tra " ^ of_ast x  
  | Tra2     x when utf8    -> "@" ^ of_ast x
  | Tra2     x     -> "tra2 " ^ of_ast x  

  | Th      x when utf8    -> "{2}" ^ of_ast x
  | Th      x     -> "{2} " ^ of_ast x

  | Hh      x when utf8    -> "(h)" ^ of_ast x
  | Hh      x     -> "(h) " ^ of_ast x
  | Ta      x when utf8    -> "{a}" ^ of_ast x
  | Ta      x     -> "{a} " ^ of_ast x
  | Ha      x when utf8    -> "(a)" ^ of_ast x
  | Ha      x     -> "(a) " ^ of_ast x
  | Ba      x when utf8    -> "[1]" ^ of_ast x
  | Ba      x     -> "[1] " ^ of_ast x

  | Box     x when utf8    -> "[]" ^ of_ast x
  | Box     x     -> "[] " ^ of_ast x

  | Bh      x when utf8    -> "[h]" ^ of_ast x
  | Bh      x     -> "[h] " ^ of_ast x  

  | Da      x when utf8    -> "<a>" ^ of_ast x
  | Da      x     -> "<a> " ^ of_ast x

  | Dia      x when utf8    -> "<>" ^ of_ast x
  | Dia      x     -> "<> " ^ of_ast x
  

  | Dh      x when utf8    -> "<h>" ^ of_ast x
  | Dh      x     -> "<h> " ^ of_ast x
  ...
  | If (x,y,z) ->
      ...
      ^ "\nelse\n" ^ of_ast z
      ^ "\nend\n"
  | Plus (x,y)  -> "plus(" ^ of_ast x ^ "," ^ of_ast y ^ ")"                   
  | Plusa (x,y) -> "plusa(" ^ of_ast x ^ "," ^ of_ast y ^ ")"                   
  | Plush (x,y) -> "plush(" ^ of_ast x ^ "," ^ of_ast y ^ ")"                       
