...
and eval_ast (env:env) (ast:Ast.t) : Ast.t =
  ...
  | Powerset x -> begin
      Printf.printf "Aca estamos POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET POWERSET 111111 \n\n" ;
...
and eval_ast_formula (env:env) (ast:Ast.t) : Ast.t =
  ...
  | Bigand (vars,sets,when_optional,body) ->    
    ...
    | Ba Top    -> Bottom
    | Ba Bottom -> Top
    | Ba x      ->  let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i,$j"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0..2],[0..2])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r($i,$j) => "^string_of_ast(x)^"($i,$j)"))                    

    | Bh Top    -> Bottom
    | Bh Bottom -> Top
    | Bh x      ->  let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0..2])"))];
                    Bigand(!vars2a,!vars2b,None,Parse.parse_sat("r($i) => "^string_of_ast(x)^"($i)"))                    
  ...
  | Da x      ->  let vars2a = ref [] in vars2a := !vars2a @ [ (Parse.parse_sat ("$i"))]; 
                    let vars2b = ref [] in vars2b := !vars2b @ [ (Parse.parse_sat ("xx([0..2])"))];
                    Bigor(!vars2a,!vars2b,None,Parse.parse_sat("r($i) => "^string_of_ast(x)^"($i)"))
  ...
  | e -> raise_with_loc ast ("tra2 + this expression is not a formula: " ^ string_of_ast e ^"\n")
