...
let string_of_lang =
 ...
 type solve_opts ={
  count: bool;
  limit: int;
  states: int;  (* NUmber of Worlds passing from TouisPlan*)  
  interactive: bool;
...
type transl_opt = { 
  linter: bool; 
  table: (string * out_channel) option; 
  debug_dimacs: bool 
  }  
  ...
  try
    ...
    in begin
      ...
      | Sat, Solve {equiv=Some (input2,input2_f); _} ->
        Printf.printf "aca_EL INICIO 00003\n";      
      ...
      | Sat, Solve {equiv=None; limit; states; count=false; _} ->
      Printf.printf "A. aca_EL INICIO SAT -- SOLVE - JFDX STATES : %s \n" (string_of_int states) ;  
      
      let string_of_command5() =
        let  tmp_file = "tempo.txt" in
        let _ = Sys.command @@ "rm  ../sdata/00_cp_pre_syntax.txt" in
      ...
      let string_of_command5() =
        let  tmp_file = "tempo.txt" in
        let _ = Sys.command @@ "rm  ../sdata/00_delta0_1.txt" in
        let chan = open_in tmp_file in
        close_in chan
      in
      string_of_command5(); 
      ...
      let string_of_command5() =
        let  tmp_file = "tempo.txt" in
        let _ = Sys.command @@ "rm  ../sdata/00_delta0_2.txt" in
        let chan = open_in tmp_file in
        close_in chan
      in
      string_of_command5(); 
      ...
      let string_of_command5() =
        let  tmp_file = "tempo.txt" in
        let _ = Sys.command @@ "rm  ../sdata/00_delta0_3.txt" in
        let chan = open_in tmp_file in
        close_in chan
      in
      string_of_command5(); 
      ...
      let string_of_command5() =
        let  tmp_file = "tempo.txt" in
        let _ = Sys.command @@ "rm  ../sdata/00_type_counters.txt" in
        let chan = open_in tmp_file in
        close_in chan
      in
      string_of_command5(); 
      ...
      let () =
        let ic = open_in "../sdata/00_delta0.txt" in

        let string_of_command () =
          let tmp_file = "../sdata/00_delta0_ini.txt" in
          let _ = Sys.command @@ "cat ../sdata/00_delta0.txt >"^ tmp_file  in
          let chan = open_in tmp_file in
          close_in chan
          in
          string_of_command();   
          
          let returncodeg = (Sys.command "sed -i '$!s/$/,/' ../sdata/00_delta0_ini.txt") in
          flush stdout; flush stderr;
          if returncodeg == 0 then Printf.printf ""
          else begin Printf.printf "[Error %d] not possible to generate file solvedatacp/goal 1.txt \n" returncodeg; exit returncodeg; end;





          let returncodeg = (Sys.command "sed -i -e '1s/^/$Delta0   	= [/' ../sdata/00_delta0_ini.txt") in
          flush stdout; flush stderr;
          if returncodeg == 0 then Printf.printf ""
          else begin Printf.printf "[Error %d] not possible to generate file solvedatacp/goal 1.txt \n" returncodeg; exit returncodeg; end;

          let returncodeg2 = (Sys.command "sed -i -e '$a]' ../sdata/00_delta0_ini.txt") in
          flush stdout; flush stderr;
          if returncodeg2 == 0 then Printf.printf ""
          else begin Printf.printf "[Error %d] not possible to generate file solvedatacp/goal 1.txt \n" returncodeg; exit returncodeg; end;

        Printf.printf "limit ==> %i \n" limit;

        try
          while true do
            let line = input_line ic in

            if (contains line "not") then begin
                                          incr ct2;
                                          let string_of_command () =
                                            let tmp_file = "../sdata/00_delta0_2.txt" in
                                            let _ = Sys.command @@ "echo \\$Delta0_2\\("^(string_of_int !ct2)^"\\) =  ['"^ line ^"'] >>" ^ tmp_file in
                                             
                                            let ini = ((String.index line 't') + 1) in
                                            let fin = ((String.length line) - (ini + 1)) in
                                            let neg = strip(String.sub line ini fin) in  
                                            let _ = Sys.command @@ "echo \\$Delta0_2\\("^(string_of_int !ct2)^",1\\) =  ['\""^ neg ^"\"'] >>" ^ tmp_file in
            ...
            else if (contains line "=>") then begin
                                          incr ct3;
                                          let string_of_command () =
                                            let tmp_file = "../sdata/00_delta0_3.txt" in
                                            let _ = Sys.command @@ "echo \\$Delta0_3\\("^(string_of_int !ct3)^"\\) =  ['"^ line ^"'] >>" ^ tmp_file in


                                            let precedence = String.sub line 0 ((String.index line '=') - 1) in  
                                            let _ = Sys.command @@ "echo \\$Delta0_3\\("^(strip (string_of_int !ct3))^",1\\) =  ['"^ precedence ^"\"'] >>" ^ tmp_file in

                                            let ini = ((String.index line '>') + 2) in
                                            let fin = ((String.length line) - (ini+1)) in

                                            let con = strip(String.sub line ini fin) in  
                                                  let _ = Sys.command @@ "echo \\$Delta0_3\\("^(string_of_int !ct3)^",2\\) =  ['\""^ con ^"\"'] >>" ^ tmp_file in
                                            let chan = open_in tmp_file in
                                            close_in chan
                                            in
                                            string_of_command();
                          ...
                          incr ct1;
                          let string_of_command () =
                            let tmp_file = "../sdata/00_delta0_1.txt" in
                            let _ = Sys.command @@ "echo \\$Delta0_1\\("^(string_of_int !ct1)^"\\) =  ['"^ line ^"'] >>" ^ tmp_file in
        ...
        with End_of_file ->
          ...
          let string_of_command () =
            let tmp_file = "../sdata/00_type_counters.txt" in
            let _ = Sys.command @@ "echo \\$n1 ="^(string_of_int !ct1)^">"^tmp_file in
            let _ = Sys.command @@ "echo \\$n2 ="^(string_of_int !ct2)^">>"^tmp_file in
            let _ = Sys.command @@ "echo \\$n3 ="^(string_of_int !ct3)^">>"^tmp_file in
            let chan = open_in tmp_file in
            close_in chan
            in
            string_of_command();         
          ...
          let string_of_command () =
            let tmp_file = "../sdata/00_final_full.txt" in
            let _ = Sys.command @@ "cat ../sdata/00_type_counters.txt ../sdata/00_delta0_ini.txt ../sdata/00_delta0_1.txt ../sdata/00_delta0_2.txt ../sdata/00_delta0_3.txt ../sdata/00_axioms.touist > "^tmp_file in
            let chan = open_in tmp_file in
            close_in chan
            in
            string_of_command(); 

      let input2 = "../sdata/00_final_full.txt" in
      let read_whole_file filename =
        let ch = open_in filename in
        let input_text2 = really_input_string ch (in_channel_length ch) in
        let replace input output = Str.global_replace (Str.regexp_string input) output in         
        let _ = Parse.parse_sat ~debug_syntax ~filename:input (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input (replace "xx" "" (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input input_text2 |> Eval.eval |> CnfExit.ast_to_cnf))))))) |> SatSolve.minisat_clauses_of_cnf
        in 
  
        close_in ch in
        read_whole_file input2;
      

      
      | Sat, Translate {linter=false; _} -> (* B. solve not asked: print the Sat file *)
        Printf.printf "B. solve not asked \n"; 
        let string_of_command5() =
          let  tmp_file = "tempo.txt" in
          let _ = Sys.command @@ "rm  ../sdata/00_cp_pre_syntax.txt" in
          let chan = open_in tmp_file in
        ...
        let start = Unix.gettimeofday () in
        let replace input output = Str.global_replace (Str.regexp_string input) output in     
        Printf.printf "Starting the process \n\n\n";

        let cls,tbl = Parse.parse_sat ~debug_syntax ~filename:input (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input  (replace "xx" "" (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input (string_of_ast(Parse.parse_sat ~debug_syntax ~filename:input input_text |> Eval.eval |> Cnf.ast_to_cnf))|> EvalTra.eval))) |> CnfTra.ast_to_cnf)) |> Eval.eval |> Cnfsat.ast_to_cnf |> SatSolve.minisat_clauses_of_cnf
        ...
        let models =
          (cls,tbl) |> SatSolve.solve_clauses ~verbose:(common_opt.verbose>0)
            ~print:print_model
            ~continue:(if limit = 999 then continue_limit else continue_limit)
        in

        (if common_opt.verbose>0 then Printf.eprintf "== solve time: %f sec\n" (Unix.gettimeofday () -. start));

        (match SatSolve.ModelSet.cardinal models with
        | 0 -> Printf.fprintf stderr "\n"; exit_with UNSAT
        ...
        | i -> (* case where we already printed models in [solve_clause] *)
          Printf.fprintf output_f
            ...
            exit_with OK)
        | _  , _  -> failwith "--solver not compatible with --smt"
...
let solve_flags =
  ...
  and limit =
    Arg.(value & opt int 1 & info ["limit"] ~docs ~docv:"N" ~doc:"\
      ...
      small problems.")
  and states =
      Arg.(value & opt int 1 & info ["states"] ~docs ~docv:"N" ~doc:"\
        (with $(b,--solve)) Instead of one model, return $(docv) models if
        they exist. With $(i,0), return every possible model. WARNING: the
        number of model can be extremely high; this option is intended for
        small problems.")      
  ...
  in let check_solve_opts solv extsolv limit states count interac equiv hidden =
       match solv,extsolv,limit,states, count,interac,equiv,hidden with
       | false   ,_      ,_    ,_    ,_    ,_      ,_    ,true  -> `Error (false,"--show-hidden only compatible with --solve or --solver")
       | true    ,Some _ ,_    ,_    ,_    ,_      ,_    ,_     -> `Error (false,"--solve and --solver cannot be used simultaneously")
       | false   ,None   ,1    ,_    ,false,false  ,None ,false -> `Ok None
       | false   ,Some c ,1    ,_    ,false,false  ,None ,hidden-> `Ok (Some (SolveExt {cmd=c;hidden}))
       | false   ,_      ,_    ,_    ,_    ,_      ,_    ,_     -> `Error (false,"the args {--limit,--count,--interactive,--equiv} can only be used with --solve")
       | true    ,_      ,limit,states,count,interac,equiv,hidden-> `Ok (Some (Solve {limit; states; count; interactive=interac; equiv; hidden}))

  in Term.(ret (const check_solve_opts $ solve_flag $ solvext_flag $ limit $ states $ count_flag $ interactive $ equiv $ hidden))
